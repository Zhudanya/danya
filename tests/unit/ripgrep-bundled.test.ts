...
test('uses DANYA_RIPGREP_PATH when set', () => {
  ...
  try {
    ...
    setEnv({ DANYA_RIPGREP_PATH: fakeRg })
...
test('throws a helpful error when DANYA_RIPGREP_PATH points to a missing file', () => {
  const dir = mkdtempSync(join(tmpdir(), 'kode-rg-missing-'))
  try {
    setEnv({
      DANYA_RIPGREP_PATH: join(dir, 'does-not-exist-rg'),
    })

    expect(() => getRipgrepPath()).toThrow(
      /DANYA_RIPGREP_PATH points to a missing file/i,
    )
  } finally {
    rmSync(dir, { recursive: true, force: true })
  }
