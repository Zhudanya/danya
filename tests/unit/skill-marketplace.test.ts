...
describe('skill marketplace (local .danya-plugin/marketplace.json)', () => {
  ...
  test('add marketplace + install/uninstall skills into user dir', async () => {
    await withEnv({ DANYA_CONFIG_DIR: join(homeDir, '.danya') }, async () => {
      ...
      expect(existsSync(join(homeDir, '.danya', 'skills', 'pdf'))).toBe(true)
      expect(existsSync(join(homeDir, '.danya', 'skills', 'xlsx'))).toBe(true)
      ...
      expect(existsSync(join(homeDir, '.danya', 'skills', 'pdf'))).toBe(false)
      expect(existsSync(join(homeDir, '.danya', 'skills', 'xlsx'))).toBe(false)
    })
  })

  test('project install writes to ./.danya/skills based on agent cwd', async () => {
    await withEnv({ DANYA_CONFIG_DIR: join(homeDir, '.danya') }, async () => {
      ...
      expect(install.installedSkills.sort()).toEqual(['pdf', 'xlsx'])
      expect(existsSync(join(projectDir, '.danya', 'skills', 'pdf'))).toBe(true)
      expect(existsSync(join(projectDir, '.danya', 'skills', 'xlsx'))).toBe(true)
    })
  })
