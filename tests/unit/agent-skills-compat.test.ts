...
describe('Agent Skills compatibility (discovery + prompt)', () => {
  ...
  test('loads .danya/skills/<name>/SKILL.md and splits allowed-tools string', async () => {
    await withEnv(
      {
        DANYA_CONFIG_DIR: join(homeDir, '.danya'),
        DANYA_SKILLS_STRICT: undefined,
      },
      async () => {
        const skillDir = join(projectDir, '.danya', 'skills', 'test-skill')
  ...
  test('accepts lowercase skill.md when SKILL.md is missing', async () => {
    await withEnv(
      ...
      async () => {
        const skillDir = join(projectDir, '.danya', 'skills', 'lower-skill')
        mkdirSync(skillDir, { recursive: true })
        writeFileSync(
          join(skillDir, 'skill.md'),
          [
  ...
  test('strict mode skips skills whose frontmatter name mismatches directory', async () => {
    await withEnv(
      { DANYA_CONFIG_DIR: join(homeDir, '.danya'), DANYA_SKILLS_STRICT: '1' },
      async () => {
        const skillDir = join(projectDir, '.danya', 'skills', 'dir-name')
        mkdirSync(skillDir, { recursive: true })
        writeFileSync(
          join(skillDir, 'SKILL.md'),
          [
  ...
  test('SkillTool.prompt includes skill location path when available', async () => {
    await withEnv(
      ...
      async () => {
        const skillDir = join(projectDir, '.danya', 'skills', 'alpha')
        mkdirSync(skillDir, { recursive: true })
