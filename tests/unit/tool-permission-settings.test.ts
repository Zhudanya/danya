...
describe('tool permission settings (multi-source load/merge/persist)', () => {
  ...
  test('loads .danya/settings.json + .danya/settings.local.json across sources', () => {
    writeJson(join(homeDir, '.danya', 'settings.json'), {
      permissions: { allow: ['Bash(ls:*)'] },
    })
    writeJson(join(projectDir, '.danya', 'settings.json'), {
      permissions: { allow: ['Bash(git:*)'] },
    })
    writeJson(join(projectDir, '.danya', 'settings.local.json'), {
  ...
  test('migrates legacy .claude settings to .danya when .danya is missing', () => {
    ...
    const migratedPath = join(projectDir, '.danya', 'settings.local.json')
  ...
  test('session updates are not persisted; policySettings is never persisted', () => {
    ...
    expect(existsSync(join(projectDir, '.danya', 'settings.local.json'))).toBe(
      false,
    )
  ...
  test('persists allow rules to localSettings (.danya/settings.local.json)', () => {
    ...
    const settingsPath = join(projectDir, '.danya', 'settings.local.json')
