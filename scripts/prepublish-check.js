...
// Check cli.js is executable (skip on Windows — npm handles permissions on install)
if (process.platform !== 'win32') {
  ...
  console.log('⏭ Skipping executable check on Windows (npm handles permissions on install)');
}

// Check package.json
const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8'));

if (!pkg.bin || !pkg.bin.danya) {
  console.error('❌ Missing bin.danya field in package.json');
  process.exit(1);
}
