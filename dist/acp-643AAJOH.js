...
import {
  init_toolPermissionSettings,
  loadToolPermissionContextFromDisk,
  persistToolPermissionUpdateToDisk
} from "./chunk-M7TET4KT.js";
import "./chunk-BGTVUX52.js";
import {
  applyToolPermissionContextUpdates,
  init_toolPermissionContext
} from "./chunk-FJF3DB5L.js";
...
var DanyaAcpAgent = class {
  ...
  async handleSessionPrompt(params) {
    ...
    if ((process.env.DANYA_ACP_ECHO ?? process.env.KODE_ACP_ECHO) === "1") {
      await this.handleDanyaMessage(session, createAssistantMessage(promptText));
      persistAcpSessionToDisk(session);
    ...
    try {
      for await (const m of query(baseMessages, session.systemPrompt, session.context, canUseTool, {
        ...
        await this.handleDanyaMessage(session, m);
      }
  ...
  async handleDanyaMessage(session, m) {
  ...
  createAcpCanUseTool(session) {
    const timeoutMs = (() => {
      const raw = process.env.DANYA_ACP_PERMISSION_TIMEOUT_MS ?? process.env.KODE_ACP_PERMISSION_TIMEOUT_MS;
