...
async function configureSessionPlugins(args) {
  ...
  const { reloadCustomCommands } = await import("./customCommands-B4T2RCKX.js");
  reloadCustomCommands();
  const { getCommands } = await import("./commands-MYAAJT2Q.js");
  getCommands.cache.clear?.();
  const { getClients, getMCPTools } = await import("./mcp-GVDPJ4CZ.js");
  getClients.cache?.clear?.();
  getMCPTools.cache?.clear?.();
  const { clearOutputStyleCache } = await import("./outputStyles-TUKZEB7I.js");
