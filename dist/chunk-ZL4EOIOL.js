...
var TODO_STORAGE_KEY, TODO_CONFIG_KEY, DEFAULT_CONFIG, todoCache, cacheTimestamp, CACHE_TTL;
var init_todoStorage = __esm({
  "src/utils/session/todoStorage.ts"() {
    init_sessionState();
    init_storage();
    TODO_STORAGE_KEY = "todos";
    TODO_CONFIG_KEY = "todoConfig";
    DEFAULT_CONFIG = {
      maxTodos: 100,
...
var SystemReminderService, systemReminderService, generateSystemReminders, emitReminderEvent, resetReminderSession;
var init_systemReminder = __esm({
  "src/services/system/systemReminder.ts"() {
    init_todoStorage();
    init_debugLogger();
    init_log();
    SystemReminderService = class {
      sessionState = {
        lastTodoUpdate: 0,
        lastFileAccess: 0,
...
${content}
    ...
    systemReminderService = new SystemReminderService();
    generateSystemReminders = (hasContext = false, agentId) => systemReminderService.generateReminders(hasContext, agentId);
    emitReminderEvent = (event, context) => systemReminderService.emitEvent(event, context);
    resetReminderSession = () => systemReminderService.resetSession();
  }
});
