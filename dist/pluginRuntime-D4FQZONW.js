...
async function configureSessionPlugins(args) {
  ...
  const { reloadCustomCommands } = await import("./customCommands-ABS5TGJU.js");
  reloadCustomCommands();
  const { getCommands } = await import("./commands-2A435OZ6.js");
  getCommands.cache.clear?.();
  const { getClients, getMCPTools } = await import("./mcp-42SCPKEC.js");
  getClients.cache?.clear?.();
  getMCPTools.cache?.clear?.();
  const { clearOutputStyleCache } = await import("./outputStyles-OFDTXSS6.js");
