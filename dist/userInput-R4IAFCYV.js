...
${input}` : input;
  if (processedInput.includes("!`") || processedInput.includes("@")) {
    try {
      const { executeBashCommands } = await import("./customCommands-LJE2M4SA.js");
      if (processedInput.includes("!`")) {
        processedInput = await executeBashCommands(processedInput);
      }
      if (processedInput.includes("@")) {
        const { processMentions } = await import("./mentionProcessor-FJTAD27X.js");
