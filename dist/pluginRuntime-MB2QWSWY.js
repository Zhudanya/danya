...
async function configureSessionPlugins(args) {
  ...
  const { reloadCustomCommands } = await import("./customCommands-WKJVHEIF.js");
  reloadCustomCommands();
  const { getCommands } = await import("./commands-YMZ4KNXW.js");
  getCommands.cache.clear?.();
  const { getClients, getMCPTools } = await import("./mcp-NZNDKVSU.js");
  getClients.cache?.clear?.();
  getMCPTools.cache?.clear?.();
  const { clearOutputStyleCache } = await import("./outputStyles-RBM6FLTW.js");
  clearOutputStyleCache();
  return { plugins, errors };
}
var PluginManifestSchema;
var init_pluginRuntime = __esm({
  "src/services/plugins/pluginRuntime.ts"() {
    init_state();
    init_sessionPlugins();
