...
function getUserConfigRoots() {
  ...
  const danyaOverride = normalizeOverride(process.env.DANYA_CONFIG_DIR ?? process.env.KODE_CONFIG_DIR);
  const hasAnyOverride = Boolean(claudeOverride || danyaOverride);
  if (hasAnyOverride) {
    return dedupeStrings([claudeOverride ?? "", danyaOverride ?? ""]);
  }
  return dedupeStrings([join(homedir(), ".claude"), join(homedir(), ".danya")]);
}
function findProjectAgentDirs(cwd) {
  ...
  while (current !== home) {
    ...
    const danyaDir = join(current, ".danya", "agents");
...
function z2A(value) {
  ...
  if (normalized === null) return value === void 0 ? void 0 : [];
...
function parseAgentFromFile(options) {
  ...
  try {
    ...
    const color = typeof fm.color === "string" ? fm.color : void 0;
    let modelRaw = fm.model;
    if (typeof modelRaw !== "string" && typeof fm.model_name === "string") {
      modelRaw = fm.model_name;
    }
    let model = typeof modelRaw === "string" ? modelRaw.trim() : void 0;
    if (model === "") model = void 0;
    const forkContextValue = fm.forkContext;
    if (forkContextValue !== void 0 && forkContextValue !== "true" && forkContextValue !== "false") {
      debug.warn("AGENT_LOADER_INVALID_FORK_CONTEXT", {
    ...
    if (forkContext && model && model !== "inherit") {
      debug.warn("AGENT_LOADER_FORK_CONTEXT_MODEL_OVERRIDE", {
        filePath: options.filePath,
        model
      });
      model = "inherit";
    ...
    const tools = toolsList === void 0 || toolsList.includes("*") ? "*" : toolsList;
    const disallowedRaw = fm.disallowedTools ?? fm["disallowed-tools"] ?? fm["disallowed_tools"];
    const disallowedTools = disallowedRaw !== void 0 ? z2A(disallowedRaw) : void 0;
...
function parseAgentFromJson(agentType, value) {
  ...
  const disallowedList = parsed.data.disallowedTools !== void 0 ? z2A(parsed.data.disallowedTools) : void 0;
  const model = typeof parsed.data.model === "string" ? parsed.data.model.trim() : void 0;
  return {
    agentType,
    whenToUse: parsed.data.description,
    tools: toolsList === void 0 || toolsList.includes("*") ? "*" : toolsList,
    ...disallowedList !== void 0 ? { disallowedTools: disallowedList } : {},
...
async function loadAllAgents() {
  ...
  const pluginAgents = pluginAgentDirs.flatMap(
    (dir) => scanAgentPaths({
      dirPathOrFile: dir,
      baseDir: dir,
      source: "plugin",
...
var VALID_PERMISSION_MODES, agentJsonSchema, agentsJsonSchema, FLAG_AGENTS, BUILTIN_GENERAL_PURPOSE, BUILTIN_EXPLORE, BUILTIN_PLAN, BUILTIN_STATUSLINE_SETUP, getActiveAgents, getAllAgents, getAgentByType, getAvailableAgentTypes, watchers;
var init_loader = __esm({
  "src/utils/agent/loader.ts"() {
    ...
    agentsJsonSchema = z.record(z.string(), agentJsonSchema);
    FLAG_AGENTS = [];
    BUILTIN_GENERAL_PURPOSE = {
...
4) Reply with the exact command you set and how the user can change/remove it later.`,
    ...
    getActiveAgents = memoize(async () => {
      const { activeAgents } = await loadAllAgents();
      return activeAgents;
    });
    getAllAgents = memoize(async () => {
      const { allAgents } = await loadAllAgents();
      return allAgents;
    });
    getAgentByType = memoize(
      async (agentType) => {
        const agents = await getActiveAgents();
        return agents.find((agent) => agent.agentType === agentType);
      }
    );
    getAvailableAgentTypes = memoize(async () => {
      const agents = await getActiveAgents();
