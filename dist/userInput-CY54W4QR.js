...
${input}` : input;
  if (processedInput.includes("!`") || processedInput.includes("@")) {
    try {
      const { executeBashCommands } = await import("./customCommands-UWDVSF44.js");
      if (processedInput.includes("!`")) {
        processedInput = await executeBashCommands(processedInput);
      }
      if (processedInput.includes("@")) {
        const { processMentions } = await import("./mentionProcessor-ZA74HRQP.js");
