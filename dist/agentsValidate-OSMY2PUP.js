...
function getUserConfigRoots() {
  ...
  if (hasAnyOverride) {
    return Array.from(new Set([claudeOverride, danyaOverride].filter(Boolean)));
  }
  return [join(homedir(), ".claude"), join(homedir(), ".danya")];
}
...
function mapClaudeModelToDanya(model) {
...
function validateOneAgentFile(args) {
  ...
  const disallowed = disallowedRaw !== void 0 ? z2A(disallowedRaw) : void 0;
  if (disallowedRaw !== void 0 && disallowed === void 0) {
    issues.push({
      level: "warning",
  ...
  const model = typeof modelRaw === "string" ? modelRaw.trim() : void 0;
  ...
  const normalizedModel = model && model.length > 0 ? mapClaudeModelToDanya(model) : void 0;
  if (normalizedModel && normalizedModel !== "inherit") {
    ...
    if (!resolved.success || !resolved.profile) {
      issues.push({
        level: "error",
        message: resolved.error ?? `Model '${String(normalizedModel)}' could not be resolved`
      });
    }
  }
...
async function validateAgentTemplates(args) {
  ...
  if (args.checkTools) {
    try {
      const { getTools } = await import("./tools-VDOJRDMZ.js");
      const { getCurrentProjectConfig } = await import("./config-SPKM7XM5.js");
  ...
  const results = Array.from(markdownFiles).sort((a, b) => a.localeCompare(b)).map(
    (filePath) => validateOneAgentFile({
      filePath,
      knownToolNames
    })
  );
