...
function scopeBaseDir(scope) {
  if (scope === "user") return userDanyaDir();
...
function pluginsDir() {
  return join(userDanyaDir(), "plugins");
}
...
function installedSkillPluginsPath() {
  return join(userDanyaDir(), INSTALLED_SKILL_PLUGINS_FILE);
}
...
function readMarketplaceFromDirectory(rootDir) {
  ...
  if (!existsSync(marketplaceFile)) {
    throw new Error(
      `Marketplace file not found (expected .danya-plugin/marketplace.json or .claude-plugin/marketplace.json)`
    );
  }
...
function baseDirForInstallRecord(record) {
  if (record.scope === "user") return userDanyaDir();
...
function parseMarketplaceSourceInput(sourceInput) {
  ...
  for (const prefix of [
    ...
    if (raw.startsWith(prefix)) {
      ...
      const parsed2 = parseRefAndPath(rest);
      if (prefix === "github:") {
        return {
          source: "github",
          repo: parsed2.base.trim(),
          ...parsed2.ref ? { ref: parsed2.ref } : {},
          ...parsed2.path ? { path: parsed2.path } : {}
        };
      }
      if (prefix === "git:") {
        const repo2 = githubRepoFromUrl(parsed2.base.trim());
        if (repo2) {
          return {
            source: "github",
            repo: repo2,
...
async function tryDownloadGithubZip(repo, ref) {
  const [owner, name] = repo.split("/", 2);
  if (!owner || !name) throw new Error(`Invalid GitHub repo: ${repo}`);
  const candidates = ref.startsWith("refs/") ? [ref] : [`refs/heads/${ref}`, `refs/tags/${ref}`];
  let lastError = null;
...
async function cacheMarketplaceToTempDir(source, tempDir) {
  ...
  if (source.source === "file") {
    ...
    const out = join(tempDir, ".danya-plugin");
    ensureDir(out);
  ...
  if (source.source === "github") {
    ...
    const includePrefix = pathWithin ? `${topDir}/${pathWithin.replace(/\/+$/, "")}/` : `${topDir}/`;
...
async function addMarketplace(sourceInput) {
...
async function refreshMarketplaceAsync(name) {
  const trimmed = name.trim();
  if (!trimmed) throw new Error("Marketplace name is required");
  const config = loadKnownMarketplaces();
  const entry = config[trimmed];
...
async function refreshAllMarketplacesAsync(onProgress) {
  const config = loadKnownMarketplaces();
...
function getMarketplaceManifest(marketplaceName) {
  const config = loadKnownMarketplaces();
...
function ensurePluginInstallState() {
  ...
  for (const record of Object.values(state)) {
    ...
    if (record.kind !== "skill-pack" && record.kind !== "plugin-pack") {
      record.kind = typeof record.pluginRoot === "string" ? "plugin-pack" : "skill-pack";
    }
    if (record.isEnabled === void 0) record.isEnabled = true;
  }
  return state;
}
...
function installSkillPlugin(pluginInput, options) {
  ...
  if (existsSync(pluginManifestPath) && lstatSync(pluginManifestPath).isFile()) {
    ...
    installState[pluginSpec] = {
      ...
      pluginRoot,
      isEnabled: true,
      projectPath: scope === "user" ? void 0 : getCwd(),
...
var KNOWN_MARKETPLACES_FILE, MARKETPLACES_CACHE_DIR, INSTALLED_SKILL_PLUGINS_FILE, MarketplaceSourceSchema, MarketplacePathListSchema, MarketplacePluginSchema, MarketplaceManifestSchema, KnownMarketplacesSchema;
var init_skillMarketplace = __esm({
  "src/services/plugins/skillMarketplace.ts"() {
    init_product();
    init_state();
    init_env();
    KNOWN_MARKETPLACES_FILE = "known_marketplaces.json";
    MARKETPLACES_CACHE_DIR = "marketplaces";
    INSTALLED_SKILL_PLUGINS_FILE = "installed-skill-plugins.json";
    ...
    MarketplacePathListSchema = z.preprocess((value) => {
      if (typeof value === "string") return [value];
      return value;
    }, z.array(z.string()));
    MarketplacePluginSchema = z.object({
      name: z.string().min(1),
    ...
    MarketplaceManifestSchema = z.object({
    ...
    KnownMarketplacesSchema = z.record(
      z.string(),
      z.strictObject({
