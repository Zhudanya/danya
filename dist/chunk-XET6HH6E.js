...
async function queryLLM(messages, systemPrompt, maxThinkingTokens, tools, signal, options) {
  const { queryLLM: inner } = await import("./llm-VEOACKJL.js");
  return inner(
    messages,
    systemPrompt,
    maxThinkingTokens,
...
async function queryQuick(args) {
  const { queryQuick: inner } = await import("./llm-VEOACKJL.js");
  return inner(args);
}
async function verifyApiKey(apiKey, baseURL, provider) {
  const { verifyApiKey: inner } = await import("./llm-VEOACKJL.js");
  return inner(apiKey, baseURL, provider);
}
async function fetchAnthropicModels(apiKey, baseURL) {
  const { fetchAnthropicModels: inner } = await import("./llm-VEOACKJL.js");
