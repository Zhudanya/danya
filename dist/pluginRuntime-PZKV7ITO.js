...
async function configureSessionPlugins(args) {
  ...
  const { reloadCustomCommands } = await import("./customCommands-LJE2M4SA.js");
  reloadCustomCommands();
  const { getCommands } = await import("./commands-UUSIVNXV.js");
  getCommands.cache.clear?.();
  const { getClients, getMCPTools } = await import("./mcp-H7G3QZOV.js");
  getClients.cache?.clear?.();
  getMCPTools.cache?.clear?.();
  const { clearOutputStyleCache } = await import("./outputStyles-XGAZCIBM.js");
