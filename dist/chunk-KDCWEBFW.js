...
var agentsJsonSchema = z.record(z.string(), agentJsonSchema);
...
var getActiveAgents = memoize(async () => {
  const { activeAgents } = await loadAllAgents();
  return activeAgents;
});
var getAllAgents = memoize(async () => {
  const { allAgents } = await loadAllAgents();
  return allAgents;
});
var getAgentByType = memoize(
  async (agentType) => {
    const agents = await getActiveAgents();
    return agents.find((agent) => agent.agentType === agentType);
  }
);
var getAvailableAgentTypes = memoize(async () => {
  const agents = await getActiveAgents();
