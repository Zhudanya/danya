...
${input}` : input;
  if (processedInput.includes("!`") || processedInput.includes("@")) {
    try {
      const { executeBashCommands } = await import("./customCommands-4H5TDSCI.js");
      if (processedInput.includes("!`")) {
        processedInput = await executeBashCommands(processedInput);
      }
      if (processedInput.includes("@")) {
        const { processMentions } = await import("./mentionProcessor-HRUF4ANP.js");
