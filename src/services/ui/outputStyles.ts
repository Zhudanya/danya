...
function getUserConfigBaseDirs(): { claude: string; danya: string }[] {
  const out: { claude: string; danya: string }[] = []

  const hasAnyOverride =
    typeof process.env.CLAUDE_CONFIG_DIR === 'string' ||
    typeof process.env.DANYA_CONFIG_DIR === 'string' ||
...
function dedupeConfigBases(
  bases: Array<{ claude: string; danya: string }>,
): Array<{ claude: string; danya: string }> {
  const seen = new Set<string>()
  const out: Array<{ claude: string; danya: string }> = []
...
function loadCustomOutputStyles(options: { cwd: string }): OutputStyleDefinition[] {
  ...
  if (isSettingSourceEnabled('userSettings')) {
    ...
    for (const base of userBases) {
      for (const userBaseDir of new Set([base.claude, base.danya])) {
