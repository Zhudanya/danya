...
export function getSystemSandboxModeFromEnv(): SystemSandboxMode | null {
  const raw = process.env.DANYA_SYSTEM_SANDBOX ?? process.env.KODE_SYSTEM_SANDBOX
  if (!raw) return null
...
export function getSystemSandboxNetworkModeFromEnv(): SystemSandboxNetworkMode | null {
  const raw = process.env.DANYA_SYSTEM_SANDBOX_NETWORK ?? process.env.KODE_SYSTEM_SANDBOX_NETWORK
  if (!raw) return null
  const v = raw.trim().toLowerCase()
