...
export async function runPrintMode({
  ...
  const { danyaMessageToSdkMessage, makeSdkInitMessage, makeSdkResultMessage } =
    await import('@utils/protocol/kodeAgentStreamJson')
  const { DanyaAgentStructuredStdio } =
  ...
  const sdkUser = danyaMessageToSdkMessage(userMsg as any, sessionIdForSdk)
  if (sdkUser) {
    if (normalizedOutputFormat === 'stream-json') {
