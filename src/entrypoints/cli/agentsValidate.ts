...
function getUserConfigRoots(): string[] {
  ...
  if (hasAnyOverride) {
    return Array.from(new Set([claudeOverride, danyaOverride].filter(Boolean))) as string[]
  }
...
function mapClaudeModelToDanya(model: string): string | 'inherit' {
...
function validateOneAgentFile(args: {
  ...
  const normalizedModel =
    model && model.length > 0 ? mapClaudeModelToDanya(model) : undefined
