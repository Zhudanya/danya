...
async function parseArgs(
  ...
  program
    ...
    .action(
      async (
        ...
        const { listDanyaAgentSessions, resolveResumeSessionIdentifier } =
          await import('@utils/protocol/kodeAgentSessionResume')
        const { isUuid } = await import('@utils/text/uuid')
        const { setDanyaAgentSessionId, getDanyaAgentSessionId } =
        ...
        if (wantsContinue) {
          const latest = findMostRecentDanyaAgentSessionId(cwd)
          if (!latest) {
        ...
        if (print) {
          ...
          if (needsResumeSelector) {
            const sessions = listDanyaAgentSessions({ cwd })
  ...
  skillsCmd
    ...
    .option('--cwd <cwd>', 'The current working directory', String, cwd())
    .option('--project', 'Install into this project (.danya/...)', () => true)
    .option('--force', 'Overwrite existing installed files', () => true)
    .action(async (plugin: string, options: any) => {
      try {
        const { setCwd } = await import('@utils/state')
  ...
  skillsCmd
    ...
    .option('--cwd <cwd>', 'The current working directory', String, cwd())
    .option('--project', 'Uninstall from this project (.danya/...)', () => true)
    .action(async (plugin: string, options: any) => {
      try {
        const { setCwd } = await import('@utils/state')
        await setCwd(options.cwd ?? cwd())
    ...
    .action(
      async (
        ...
        if (identifier !== undefined) {
          const { loadDanyaAgentSessionMessages } =
            await import('@utils/protocol/kodeAgentSessionLoad')
          const { resolveResumeSessionIdentifier } =
            await import('@utils/protocol/kodeAgentSessionResume')
          const { setDanyaAgentSessionId } =
            await import('@utils/protocol/kodeAgentSessionId')
