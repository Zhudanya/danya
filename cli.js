...
function run(cmd, args) {
  const result = spawnSync(cmd, args, {
    stdio: 'inherit',
    env: { ...process.env, DANYA_PACKAGED: process.env.DANYA_PACKAGED || process.env.KODE_PACKAGED || '1' },
  })
  if (result.error) {
    throw result.error
  }
...
function main() {
  ...
  process.stderr.write(
    [
      '❌ Danya is not runnable on this system.',
      '',
      'Tried:',
      ...
      '',
      'Fix:',
      '- Reinstall (ensure network access), or set DANYA_BINARY_BASE_URL to a mirror',
