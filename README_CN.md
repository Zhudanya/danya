...
<p align="center">
  <a href="README.md">English</a> | 中文 | <a href="https://zhudanya.github.io/posts/danya-complete-guide/">完全使用指南</a>
</p>
...
### 5 个新增游戏专用工具
- **CppServerBuild** — CMake/Ninja 编译、cppcheck 检查、ctest 测试（C++ 服务器）
- **JavaServerBuild** — Maven/Gradle 编译、checkstyle 检查、JUnit 测试（Java 服务器）
- **NodeServerBuild** — tsc 编译、ESLint 检查、Jest/Vitest 测试（Node.js 服务器）
- **PerfLint** — 热路径静态性能分析（Unity/Unreal/Godot）
- **ProtoCompat** — Proto 破坏性变更检测（基于 git diff）
- **ShaderCheck** — Shader 验证（变体爆炸、采样器限制、语法检查）

### 增强工具
- **AssetCheck** — 新增 Unreal 支持（命名规范、大小警告、引用验证）+ 深层嵌套检测（>10 层）+ 非活跃大对象检测（所有引擎）

### 新增服务器语言支持
- **C++ 服务器** — CMakeLists.txt 检测、引擎变体、构建工具、模板包
- **Java 服务器** — pom.xml/build.gradle + 游戏关键词检测、引擎变体、构建工具、模板包
- **Node.js 服务器** — package.json + colyseus/socket.io 检测、引擎变体、构建工具、模板包

### 新增 Hook
- **AssetGuard** — 预提交 Hook，拦截未被 Git LFS 跟踪的大二进制文件（>5MB）
...
- **工具系统** — 可扩展的架构（22 通用 + 18 游戏专用 = 40 个工具）

### 游戏开发专用
- **游戏引擎感知** — 自动识别 Unity / UE / Godot / Go 服务器 / C++ 服务器 / Java 服务器 / Node.js 服务器，注入引擎领域知识
...
7 套引擎模板内置（Unity、Go Server、Unreal、Godot、C++ Server、Java Server、Node.js Server），根据检测结果自动选择。用户可以自由增删改规则，Danya 不会覆盖用户的自定义内容。
...
| **CppServerBuild** | CMake/Ninja 编译、cppcheck 检查、ctest 测试 | C++ Server |
| **JavaServerBuild** | Maven/Gradle 编译、checkstyle 检查、JUnit 测试 | Java Server |
| **NodeServerBuild** | tsc 编译、ESLint 检查、Jest/Vitest 测试 | Node.js Server |
| **ProtoCompile** | Protobuf 编译 + 桩代码生成 | 跨引擎 |
| **ProtoCompat** | Proto 破坏性变更检测（基于 git diff） | 跨引擎 |
...
| **AssetCheck** | 资源引用完整性 + 大小/嵌套检查 | Unity, Godot, UE |
| **PerfLint** | 热路径静态性能分析 | Unity, UE, Godot |
| **ShaderCheck** | Shader 验证（变体、采样器、语法） | Unity, UE, Godot |
...
- **AssetGuard（Hook）**：预提交 Hook，拦截未被 Git LFS 跟踪的大二进制文件（>5MB）
...
| **C++ 服务器** | `CMakeLists.txt` | CppServerBuild | — |
| **Java 服务器** | `pom.xml` / `build.gradle` + 游戏关键词 | JavaServerBuild | — |
| **Node.js 服务器** | `package.json` + colyseus/socket.io | NodeServerBuild | — |
